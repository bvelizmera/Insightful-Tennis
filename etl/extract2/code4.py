...
async def load_match_webpage(year: int, match_number: str):
    ...
    try:
        ...
        driver.get(response)
        await asyncio.sleep(3)  # Delay for the page to load
...
async def get_year_data(max_retries=5, retry_delay=3):
    ...
    tasks = []
    for year in years:
        tasks.append(fetch_data_for_year(year, matches, max_retries, retry_delay))

    # Run all year tasks concurrently
    await asyncio.gather(*tasks)

async def fetch_data_for_year(year, matches, max_retries, retry_delay):
    matches_dict = {}
    ...
    await save_to_json(matches_dict, year_filename)
